...
const form = reactive({
    ...
    user_id : props.interaction.user.id,
    user_name : props.interaction.user.name,
    id : props.interaction.id,
    content : props.interaction.content,
    supported_types : props.interaction.supported_types,
    status : props.interaction.status,
});

const updateInteraction = id => {
    router.post(route('interactions.update', id), form, {
        onSuccess: () => {
            emit('close');
        },
        onError: () => {
...
const deleteInteraction = id => {
    ...
    router.post(route('interactions.destroy', id), {
    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="modal relative overflow-visible z-50 bg-white sm:rounded-lg">
            <div class="fixed top-0 left-0 m-3">
                <h1 class="sm:text-xl text-xl font-medium title-font mb-2 text-gray-900">対応履歴更新</h1>
            </div>
            ...
            <div class="p-6 text-gray-900 mt-6">
                <section class="text-gray-600 body-font relative flex flex-col">
                    <form @submit.prevent="updateInteraction(form.id)">
