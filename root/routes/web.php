...
Route::middleware('auth')
    ...
    ->group(function() {
        ...
        Route::post('{customer}/destroy', 'destroy')->name('destroy')->middleware('is_admin'); # 顧客削除処理(staff削除不可)
    });
...
Route::middleware('auth')
    ...
    ->group(function() {
        Route::get('/{id}', 'index')->name('index'); # 対応履歴一覧
        Route::post('/', 'store')->name('store'); # 新規対応履歴登録処理
        Route::post('/{interaction}/update', 'update')->name('update'); # 対応履歴更新処理
        Route::post('{interaction}/destroy', 'destroy')->name('destroy')->middleware('is_admin'); # 対応履歴削除処理(staff削除不可)
    });
